...
''' 
    ...
    Notes:  
    IP:The function processes an upload log entry and a task ID to handle case holds. the upload log entry is containing:
    
        "file_upload_seq" (Unique identifier for the uploaded file)
        "File_Path" (Location of the file to process)
        "File_Type" (Type of the file)
        
    The function move and processes the CSV files containing case_hold records with not more than values per row:
    ...
    OP:output appends additional messages from the system ,  error messages if given values are empty or wrong to the CSV and update the reletive case_details document.
        ...
        Example output(csv): 12345,1234567890,011xxxxxxx,Document verification pending 
                             56372,          ,011xxxxxxx,Complience issue 
                                  ,1242345646,          ,missing payement details 
                                 
            
            Error messages: Invalid account number,invalid case_id,invalid telephone_number(if 3 of them missing or failed the validation), Reject reason is missing  
        ...
        when the valid records are found(records that contains valid account number or case_id or Telephone number and Hold reason) the system will update the case status 
        to "Abundant" and the status_description with the Hold reason in the case_details document.                
'''
#endregion
...
def read_case_hold(file_upload_log_entry, task_id, file_upload_seq, file_type,file_path):
    logger = get_logger("Case Hold")
    ...
    while True:
        ...
        new_file_path = os.path.join(config.get("case_hold_path"), new_file_name)
        if not os.path.exists(new_file_path):
            break
        time.sleep(1)

    os.makedirs(config.get("case_hold_path"), exist_ok=True)
    ...
    try:
        ...
        error_file_path = os.path.join(config.get("case_hold_path"), error_file_name)
        ...
        with open(new_file_path, "r", encoding="utf-8-sig") as record_file, \
            ...
            for row in file_reader:
                ...
                try:
                    ...
                    case_id, account_number, telephone_number, hold_reason = map(str.strip, row[:4])
                    
                    #validate the row values
                    match (case_id, account_number, telephone_number, hold_reason):
                        case (_, _, _, ""):
                            raise ValidationError(total_record_count, "Hold Reason", "|Hold reason is missing")
                    ...
                    case_details.update_one(
                        ...
                        {"$set": {
                            "case_current_status": "Abundant",
                            "case_status.0.case_status": "Abundant",
                            "case_status.0.status_reason": hold_reason
                        }}
                    )
                    logger.info(f"Updated case to Abundant status with hold reason: {hold_reason}")
    ...
    logger.info(f"Task {task_id}: Case hold process completed.")
    ...
    logger.info(f"Task {task_id} status set to 'Completed' in system_tasks.")
    ...
    logger.info(f"Task {task_id} status set to 'Completed' in system_tasks_inprogress.")
